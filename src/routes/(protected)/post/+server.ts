...
export const POST: RequestHandler = async ({
    ...
    try {
        const newItem: Item = {
            user_id: 0, // Replace with actual user ID from session or context
        ...
        //TODO: check if category is exists or create new one
        ...
        if (formData.images) {
            const imageInsertions = formData.images.map(
                (image: string | File) => ({
